...
+(FLTFullScreenMessage*)fromMap:(NSDictionary*)dict {
  ...
  result.fullscreen = dict[@"fullscreen"];
  if ((NSNull *)result.fullscreen == [NSNull null]) {
    result.fullscreen = nil;
  }
  return result;
}
-(NSDictionary*)toMap {
  return [NSDictionary dictionaryWithObjectsAndKeys:(self.textureId ? self.textureId : [NSNull null]), @"textureId", (self.fullscreen ? self.fullscreen : [NSNull null]), @"fullscreen", nil];
}
@end
...
void FLTVideoPlayerApiSetup(id<FlutterBinaryMessenger> binaryMessenger, id<FLTVideoPlayerApi> api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [FlutterBasicMessageChannel
        messageChannelWithName:@"dev.flutter.pigeon.VideoPlayerApi.setFullScreen"
        binaryMessenger:binaryMessenger];
    if (api) {
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        FlutterError *error;
        FLTFullScreenMessage *input = [FLTFullScreenMessage fromMap:message];
        [api setFullScreen:input error:&error];
        callback(wrapResult(nil, error));
      }];
    }
    else {
